...
const dashHTML = `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Deed</title>
<link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;700&display=swap" rel="stylesheet">
<style>:root{--bg:#1a1410;--bg2:#241e18;--bg3:#2e261e;--rust:#e8753a;--leather:#a0845c;--cream:#f0e6d3;--cd:#bfb5a3;--cm:#7a7060;--gold:#d4a843;--green:#4a9e5c;--red:#c94444;--blue:#5b8dd9;--mono:'JetBrains Mono',monospace}*{margin:0;padding:0;box-sizing:border-box}body{background:var(--bg);color:var(--cream);font-family:var(--mono);line-height:1.5}.hdr{padding:1rem 1.5rem;border-bottom:1px solid var(--bg3);display:flex;justify-content:space-between;align-items:center}.hdr h1{font-size:.9rem;letter-spacing:2px}.hdr h1 span{color:var(--rust)}.main{padding:1.5rem;max-width:960px;margin:0 auto}.stats{display:grid;grid-template-columns:repeat(3,1fr);gap:.5rem;margin-bottom:1rem}.st{background:var(--bg2);border:1px solid var(--bg3);padding:.6rem;text-align:center}.st-v{font-size:1.2rem;font-weight:700}.st-l{font-size:.5rem;color:var(--cm);text-transform:uppercase;letter-spacing:1px;margin-top:.15rem}.toolbar{display:flex;gap:.5rem;margin-bottom:1rem;align-items:center}.search{flex:1;padding:.4rem .6rem;background:var(--bg2);border:1px solid var(--bg3);color:var(--cream);font-family:var(--mono);font-size:.7rem}.search:focus{outline:none;border-color:var(--leather)}.lic{background:var(--bg2);border:1px solid var(--bg3);padding:.8rem 1rem;margin-bottom:.5rem;transition:border-color .2s}.lic:hover{border-color:var(--leather)}.lic-top{display:flex;justify-content:space-between;align-items:flex-start;gap:.5rem}.lic-product{font-size:.85rem;font-weight:700}.lic-holder{font-size:.7rem;color:var(--cd);margin-top:.1rem}.lic-key{font-size:.55rem;color:var(--cm);margin-top:.2rem;background:var(--bg);padding:.2rem .4rem;border:1px solid var(--bg3);word-break:break-all;font-family:var(--mono)}.lic-meta{font-size:.55rem;color:var(--cm);margin-top:.3rem;display:flex;gap:.5rem;flex-wrap:wrap;align-items:center}.lic-actions{display:flex;gap:.3rem;flex-shrink:0}.badge{font-size:.5rem;padding:.12rem .35rem;text-transform:uppercase;letter-spacing:1px;border:1px solid}.badge.active{border-color:var(--green);color:var(--green)}.badge.expired{border-color:var(--red);color:var(--red)}.badge.revoked{border-color:var(--cm);color:var(--cm)}.badge.trial{border-color:var(--gold);color:var(--gold)}.btn{font-size:.6rem;padding:.25rem .5rem;cursor:pointer;border:1px solid var(--bg3);background:var(--bg);color:var(--cd);transition:all .2s}.btn:hover{border-color:var(--leather);color:var(--cream)}.btn-p{background:var(--rust);border-color:var(--rust);color:#fff}.btn-sm{font-size:.55rem;padding:.2rem .4rem}.modal-bg{display:none;position:fixed;inset:0;background:rgba(0,0,0,.65);z-index:100;align-items:center;justify-content:center}.modal-bg.open{display:flex}.modal{background:var(--bg2);border:1px solid var(--bg3);padding:1.5rem;width:460px;max-width:92vw}.modal h2{font-size:.8rem;margin-bottom:1rem;color:var(--rust);letter-spacing:1px}.fr{margin-bottom:.6rem}.fr label{display:block;font-size:.55rem;color:var(--cm);text-transform:uppercase;letter-spacing:1px;margin-bottom:.2rem}.fr input,.fr select{width:100%;padding:.4rem .5rem;background:var(--bg);border:1px solid var(--bg3);color:var(--cream);font-family:var(--mono);font-size:.7rem}.fr input:focus,.fr select:focus{outline:none;border-color:var(--leather)}.row2{display:grid;grid-template-columns:1fr 1fr;gap:.5rem}.acts{display:flex;gap:.4rem;justify-content:flex-end;margin-top:1rem}.empty{text-align:center;padding:3rem;color:var(--cm);font-style:italic;font-size:.75rem}</style></head><body>
...
function renderStats(){var total=items.length;var active=items.filter(function(l){return l.status==='active'}).length;var seats=items.reduce(function(s,l){return s+(l.max_seats||0)},0);
document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+total+'</div><div class="st-l">Licenses</div></div><div class="st"><div class="st-v" style="color:var(--green)">'+active+'</div><div class="st-l">Active</div></div><div class="st"><div class="st-v">'+seats+'</div><div class="st-l">Total Seats</div></div>';}
...
h+='<div class="lic-meta"><span class="badge '+l.status+'">'+l.status+'</span>';
...
h+='<div class="fr"><label>Product *</label><input id="f-prod" value="'+esc(i.product)+'"></div>';
h+='<div class="fr"><label>License Key</label><input id="f-key" value="'+esc(i.key)+'" placeholder="auto-generated if empty"></div>';
...
['active','trial','expired','revoked'].forEach(function(s){h+='<option value="'+s+'"'+(i.status===s?' selected':'')+'>'+s+'</option>';});
